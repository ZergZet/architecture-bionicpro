...
const KEYCLOAK_PUBLIC_URL   = process.env.KEYCLOAK_PUBLIC_URL   || 'http://localhost:8081';
...
const REPORTS_API_URL       = process.env.REPORTS_API_URL       || 'http://reports:8000';
...
async function refreshTokens(session) {
  ...
  if (t.expires_at && t.expires_at > Math.floor(Date.now() / 1000) + 30) {
  ...
  session.tokens = {
    ...t,
    ...r.data,
    expires_at: Math.floor(Date.now() / 1000) + (r.data.expires_in || 300),
  };
...
app.get('/auth/login', (req, res) => {
  ...
  res.redirect(`${KEYCLOAK_PUBLIC_URL}/realms/${REALM}/protocol/openid-connect/auth?${params}`);
});
...
app.all('/api/*', async (req, res) => {
  ...
  try {
    ...
    const url  = `${REPORTS_API_URL}${path}`;
